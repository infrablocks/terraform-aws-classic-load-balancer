...
resource "aws_security_group_rule" "elb_ingress" {
  ...
  from_port = "${lookup(var.access_control[count.index], "lb_port")}"
  to_port = "${lookup(var.access_control[count.index], "lb_port")}"
  protocol = "tcp"
  cidr_blocks = ["${lookup(var.access_control[count.index], "allow_cidr")}"]
...
resource "aws_security_group_rule" "open_to_elb_ingress" {
  ...
  from_port = "${lookup(var.access_control[count.index], "instance_port")}"
  to_port = "${lookup(var.access_control[count.index], "instance_port")}"
  protocol = "tcp"
